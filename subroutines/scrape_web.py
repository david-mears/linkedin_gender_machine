...
def scrape_web(url):
    ...
    for word in company_name_list:
        company_name.append(word.capitalize())
    ...
    number_string = driver.find_element_by_class_name(
        't-20').text.split(' ')[0]
    ...
    if linkedin_employee_count > MAX_EMPLOYEE_COUNT:
        print(
            'Sorry: there are ' +
            str(linkedin_employee_count) +
            ' employees at this company.')
        print('To load all of them would take me ' +
              str((SCROLL_PAUSE_TIME *
                   linkedin_employee_count) /
                  60).split('.')[0] +
              ' minutes.')
        print(
            'I will sample the first ' +
            str(MAX_EMPLOYEE_COUNT) +
            '-ish employees only.')
    ...
    while True:
        ...
        employees_found = len(driver.find_elements_by_class_name(
    ...
    # Scrape names
    name_cards = driver.find_elements_by_class_name(
        'org-people-profile-card__profile-title')
    names = []
